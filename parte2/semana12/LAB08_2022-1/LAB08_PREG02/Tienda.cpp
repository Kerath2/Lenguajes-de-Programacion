...
void Tienda::muestra() {
  ...
  arch << "Fecha    Codigo Descripcion del Prod.                               "
          "              Cant. Precio   Total    Observaciones"
       << endl;

  for (int i = 0; lpedidos[i].GetCodigo(); i++) {
    char nombre[100];
    lpedidos[i].GetNombre(nombre);
    arch << lpedidos[i].GetFecha() << " " << lpedidos[i].GetCodigo() << " "
         << left << setw(60) << nombre << " " << right << setw(8)
         << lpedidos[i].GetCantidad() << setw(8) << lpedidos[i].GetPrecio()
         << " " << lpedidos[i].GetTotal() << endl;
  }
}

void Tienda::actualiza(int stockmin) {
  for (int i = 0; lpedidos[i].GetCodigo(); i++) {
    char cat = devuelveCat(lpedidos[i].GetDni());
    lpedidos[i].actualizaValores(stockmin, cat);
  }
}

char Tienda::devuelveCat(int dni) {
